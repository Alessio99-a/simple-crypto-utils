...
export async function decryptFile(
  ...
  try {
    ...
    const { bytesRead: lengthBytesRead } = await fileHandle.read(
      headerLengthBuf,
      0,
      4,
      0
    ...
    if (lengthBytesRead < 4) {
      throw new Error(
        `File is too small (${lengthBytesRead} bytes). Expected at least 4 bytes for header length.`
      );
    }
    ...
    if (headerLength > 10000) {
      throw new Error(
        `Header length suspiciously large (${headerLength} bytes). File may be corrupted.`
      );
    }
    ...
    if (headerBytesRead < headerLength) {
      throw new Error(
        `File is too short to contain the header. Expected ${headerLength} bytes, got ${headerBytesRead}`
      );
    }
